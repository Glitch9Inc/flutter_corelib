...
class DotFilter<T> extends StatefulWidget {
  ...
  @override
  DotFilterState<T> createState() => DotFilterState<T>();
}

class DotFilterState<T> extends State<DotFilter<T>> {
  ...
  void initState() {
    ...
    inactiveIconColor = widget.inactiveColor.brighten(0.3);
  }
    ...
    return GestureDetector(
      onTap: () {
        if (mounted) {
          setState(() {
            _toggleFilter(index);
            widget.onFiltered(currentValues);
          });
        }
      },

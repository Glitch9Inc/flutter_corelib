...
extension DynamicMapExt on Map<String, dynamic> {
  ...
  Map<String, Object> getMap(String key, {Map<String, Object>? defaultValue}) {
    ...
    if (_isNull(value)) return defaultValue ?? {};
    if (value is Map<String, Object>) return this[key] as Map<String, Object>;
  ...
  T getEnum<T extends Enum>(String key, List<T> values, {T? defaultValue}) {
    ...
    if (_isNull(value)) return defaultValue ?? values.first;
    if (value is T) return value;
    if (value is String) {
      if (value.isEmpty) return defaultValue ?? values.first;
      return values.firstWhere((element) => _getEnumName(element.toString()) == _getEnumName(value.toString()),
          orElse: () {
        print('Enum not found: $value');
  ...
  Timestamp getTimestamp(String key, {Timestamp? defaultValue}) {
    final value = this[key];
    if (_isNull(value)) return defaultValue ?? Timestamp.now();

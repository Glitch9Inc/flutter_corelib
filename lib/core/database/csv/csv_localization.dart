...
class CsvLocalization {
  ...
  static Future<void> init({
    ...
    if (supportedLocaleCodes != null) {
      ...
      CsvLocalization.supportedLocaleCodes = supportedLocales.map((e) => e.languageCode).toList();
    }
  ...
  static Future<void> load(String tableName, {Locale? loadLocale, bool? cacheAllLocales}) async {
    ...
    try {
      ...
      final csvData = csvTable.skip(1).map((e) => Map<String, dynamic>.fromIterables(headers, e.map((e) => e?.toString() ?? ''))).toList();
      ...
      for (var row in csvData) {
        ...
        for (var locale in (cacheAllLocales ?? _cacheAllLocales ? [currentLocale.value.languageCode] : supportedLocaleCodes)) {
          if (row[locale] == null) {
      ...
      _cachedLocales.addAll((cacheAllLocales ?? _cacheAllLocales ? [currentLocale.value.languageCode] : supportedLocaleCodes)
  ...
  static void loadSync(String tableName, {Locale? loadLocale}) {
    ...
    try {
      rootBundle.loadString(_buildPath(tableName)).then((data) {
        ...
        final headers = csvTable.first.map((e) => e.toString()).toList();
        final csvData =
            csvTable.skip(1).map((e) => Map<String, dynamic>.fromIterables(headers, e.map((e) => e?.toString() ?? _emptyValue))).toList();
        ...
        _cachedLocales.addAll(
            (_cacheAllLocales ? headers.sublist(1) : supportedLocaleCodes).where((locale) => !_cachedLocales.contains(locale)).toList());
      });
    } catch (e) {
      _logger.severe('Failed to load CSV table: $e');
    }
  ...
  static String getWithAutoFormatKey({required KeyType type, required String id}) {
